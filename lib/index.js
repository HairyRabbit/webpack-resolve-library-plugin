...
export default function (webpackOptions: WebpackOptions): Promise<WebpackOptions> {
  ...
  function injectOptionsForProduction () {
    return new Promise(function (resolve, reject) {
      ...
      fs.readFile(pkgJsonPath).then(pkg => {
        ...
        const externalLibs = libs.map(lib => {
          require.context()
        })
        // 
        ...
        webpackOptions.externals = Object.assign({}, externals, libs)
  ...
  function getUMDGlobalName (libname) {
    return fs.readFile(require.resolve(libname))
      .then(scripts => {
  ...
  function getCDNUrl (libname) {
    return glob(`node_modules/${libname}/+(umd|dist|build)/+(${libname}|index).+(min|production).js`)
      .then(paths => {
